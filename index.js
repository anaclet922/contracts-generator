...
app.use(express.static(path.join(__dirname, "letters")));
...
app.post("/generate-proposal", async function (req, res) {
  ...
  let content = fs.readFileSync(
    path.resolve(
      __dirname,
      "docx-templates/template-payment-proposal-" + typpol + ".docx"
    ),
    "binary"
  );
...
app.get("/download/:filename", (req, res) => {
  ...
  res.download(filePath, "downloaded-" + req.params.filename, (err) => {
    if (err) {
      res.send({
        error: err,
...
app.post("/generate-combined-proposal", async function (req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error generating combined proposal" });
  }
});
...
app.post("/generate-rejection-letter", async (req, res) => {
  let nsin = req.body.nsin;
  let all_body = req.body;

  // Load the docx file as binary content
  let content = fs.readFileSync(
    path.resolve(__dirname, "docx-templates/template-rejection-letter.docx"),
    "binary"
  );
  ...
  // file or res.send it with express for example.
  let filename = nsin + "-output.docx";
  fs.writeFileSync(path.resolve(__dirname, "letters/" + filename), buf);
...
app.get("/pdf-letter/:docxfilename", async (req, res) => {
  const filename = req.params.docxfilename;
  const ext = ".pdf";
  const inputPath = path.join(__dirname, "/letters/" + filename);
  const outputPath = path.join(__dirname, `/tmp/${filename}${ext}`);
...
app.post("/generate-payment-slip", async (req, res) => {
  let nsin = req.body.nsin;
  let all_body = req.body;

  // Load the docx file as binary content
  let content = fs.readFileSync(
    path.resolve(__dirname, "docx-templates/template-demande-de-paiement.docx"),
    "binary"
  );
  ...
  // buf is a nodejs Buffer, you can either write it to a
  // file or res.send it with express for example.
  let filename = nsin + "-slip-output.docx";
  fs.writeFileSync(path.resolve(__dirname, "payments/" + filename), buf);
...
app.get("/pdf-payment-slip/:docxfilename", async (req, res) => {
  const filename = req.params.docxfilename;
  const ext = ".pdf";
  const inputPath = path.join(__dirname, "/payments/" + filename);
  const outputPath = path.join(__dirname, `/tmp/${filename}${ext}`);
...
app.post("/generate-doc-misc", async (req, res) => {
  let nsin = req.body.nsin;
  let all_body = req.body;

  // Load the docx file as binary content
  let content = fs.readFileSync(
    path.resolve(__dirname, "docx-templates/" + all_body.template + ".docx"),
    "binary"
  );
  ...
  // buf is a nodejs Buffer, you can either write it to a
  // file or res.send it with express for example.
  let filename = nsin + all_body.template + "-output.docx";
  fs.writeFileSync(path.resolve(__dirname, "generated/" + filename), buf);
...
app.get("/download-doc-misc/:docxfilename", async (req, res) => {
  const filename = req.params.docxfilename;
  const ext = ".pdf";
  const inputPath = path.join(__dirname, "/generated/" + filename);
  const outputPath = path.join(__dirname, `/tmp/${filename}${ext}`);
